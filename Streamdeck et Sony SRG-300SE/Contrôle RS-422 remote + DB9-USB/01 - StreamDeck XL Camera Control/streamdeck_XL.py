...
from presets import enregistrer_preset, rappeler_preset, save_configuration, load_configuration, on_preset_changed
...
def update_display(deck):
    ...
    # Mettre à jour le bouton toggle pour indiquer le mode actif
    set_toggle_button(deck, "STORE" if recording_enabled else "RECALL")

    # Afficher l'état correct du bouton SAVE en fonction de config_changed
...
def streamdeck_callback(deck, key, state):
    global recording_enabled, config_changed, camera_number  # Ensure camera_number is accessible
    if state:
