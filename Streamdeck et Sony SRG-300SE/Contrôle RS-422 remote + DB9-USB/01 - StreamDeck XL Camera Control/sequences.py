...
def sequence_actions(camera_number, preset_number):
    # Étape 1: Rappel explicite du preset 16 pour la caméra 4 (plan large)
    print("Forçage du rappel du preset 16 pour la caméra 4 (plan large)")
    recall_preset(4, 16)  # Envoi explicite du preset 16
    print("Preset 16 pour la caméra 4 envoyé avec succès")  # Log ajouté ici

    # Étape 2: Temporisation de 1,5 seconde pour laisser le temps à la caméra de se caler
    time.sleep(2)

    # Étape 3: Passer la caméra 4 en Preview
    set_camera_preview(4)

    # Étape 4: Lancer la transition AUTO pour la caméra 4
    auto_transition()
    time.sleep(1.5)

    # Étape 5: Rappel du preset de la caméra active (par exemple, caméra 1)
    if camera_number != 4:  # Ne pas rappeler le preset si c'est déjà la caméra 4
    ...
    # Étape 9: Rappel explicite du preset 15 pour la caméra 4 (plan flou)
    print("Forçage du rappel du preset 16 pour la caméra 4 (plan flou)")
    recall_preset(4, 15)  # Envoi explicite du preset 16
    print("Preset 15 pour la caméra 4 envoyé avec succès")  # Log ajouté ici
